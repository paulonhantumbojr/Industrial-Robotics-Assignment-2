...
%% Path (UR3)
  ...
  q2 = deg2rad([81.5 -20.4 28.4 59.2 90 0]);
  q3 = deg2rad([81.5 -6 28.4 51 90 0]);
  ...
  q4 = deg2rad([81.5 -20.4 28.4 59.2 90 0]);
  q5 = deg2rad([168 -81.0 57.2 93.6 90 86.6]);
  q6 = deg2rad([189 -37.8 -7.6 112 90 86.6]);
  q7 = deg2rad([189 -20 -14.8 112 90 86.6]); 
  ...
  for i = 1:size(qMatrix,1)
      ...
      tr = myUR3.model.fkine(ur3Q) * transl(0,0,0.1975); 
      plate2_tverts = [plate2_verts, ones(size(plate2_verts, 1),1)] * tr';
      set(plate2_h ,'Vertices',plate2_tverts(:,1:3)) 
  ...
  new_q0 = deg2rad([0 0 0 0 0 0]);

  qWayPoints = [q7;q8;q9;new_q0];
  ...
  % Pick up and drop plate 
  q3 = deg2rad([-45 50 0 -43.2 0 -104 43.5]); 
  q4 = deg2rad([-131 32 0 -43.2 0 -104 130]);
  q5 = deg2rad([-131 60 0 -43.2 97.2 -144 14]); 
  ...
  plate2_tverts = [plate2_verts, ones(size(plate2_verts, 1),1)] * transl(-0.725,1.335,2.185)' * trotx(-pi/2);
  set(plate2_h ,'Vertices',plate2_tverts(:,1:3)) 
