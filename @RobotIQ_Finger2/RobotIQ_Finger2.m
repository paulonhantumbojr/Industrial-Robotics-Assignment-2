classdef RobotIQ_Finger2 < handle
    ...
    methods
       %% Setting the constructor for the gripper class that will hold and create the objects 
        function self = RobotIQ_Finger2(useGripper)
           if nargin < 1
              useGripper = false;
           end
...
%             self.GetAndColourFinger2();
       ...
       self.model = SerialLink(H,'name',name);
        ...
        function GetAndColourFinger2(self)%robot,workspace)
            for linkIndex = 1:self.model.n
                [ faceData, vertexData, plyData{linkIndex + 1} ] = plyread(['gripper2_finger',num2str(linkIndex),'.ply'],'tri');                
