classdef Kitchen < handle
    ...
    methods
        ...
        table_h = PlaceObject('WorkingTable.PLY',[-0.25,-1.2,-0.05]);
        ...
        cupboard_h2 = PlaceObject('platecupboard.PLY',[-0.35,-2.17,1.8]);
        verts = [get(cupboard_h2 ,'Vertices'), ones(size(get(cupboard_h2 ,'Vertices'),1),1)];
        set(cupboard_h2 ,'Vertices',verts(:,1:3))
        
        rack_h = PlaceObject('dishrack.PLY',[0.25,-2.175,1.05]);
        verts = [get(rack_h ,'Vertices'), ones(size(get(rack_h ,'Vertices'),1),1)];
        set(rack_h ,'Vertices',verts(:,1:3))
        
        worker_h = PlaceObject('worker.PLY',[0,0,1.015]);
        verts = [get(worker_h ,'Vertices'), ones(size(get(worker_h ,'Vertices'),1),1)] * troty(pi/2) * trotx(-pi/2);
        ...
        elements = {cupboard_h1, fence_h1, fence_h2, door_h, table_h, estop_h, warninglight_h, rack_h, ...
                    lightcurtain_h1, lightcurtain_h2, lightcurtain_h3, dishwasher_h, cupboard_h2, worker_h}; 
        end
    end
