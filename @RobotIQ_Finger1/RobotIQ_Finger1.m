classdef RobotIQ_Finger1 < handle
    ...
    methods
       %% Setting the constructor for the gripper class that will hold and create the objects 
        function self = RobotIQ_Finger1(useGripper)
           if nargin < 1
              useGripper = false;
           end
...
%             self.GetAndColourFinger1();
       ...
       self.model = SerialLink(H,'name',name);
        ...
        function GetAndColourFinger1(self)%robot,workspace)
            for linkIndex = 1:self.model.n
                [ faceData, vertexData, plyData{linkIndex + 1} ] = plyread(['gripper1_finger',num2str(linkIndex),'.ply'],'tri');                
