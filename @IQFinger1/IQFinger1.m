classdef IQFinger1 < handle
    ...
    methods
       %% Setting the constructor for the gripper class that will hold and create the objects 
        function self = IQFinger1(useGripper)
           if nargin < 1
              useGripper = false;
           end
            ...
            % Colour the gripper
%             self.GetAndColourFingerIQ1();
            
        end
        %% Set up the model of the Gripper
        function GetFingerIQ1(self)
            
       name = 'Finger1';
       % Set up DH Parameters for fingers
       H(1) = Link([0 0 0.02 0 0]); 
       H(2) = Link([0 0 0.015 0 0]); 
 
       % Set the joint limits
       H(1).qlim = [-20 30]*pi/180;
       H(2).qlim = [-10 40]*pi/180;
       ...
       self.model = SerialLink(H,'name',name);
        ...
        function GetAndColourFingerIQ1(self)%robot,workspace)
            for linkIndex = 1:self.model.n
                [ faceData, vertexData, plyData{linkIndex + 1} ] = plyread(['iqgripper1_finger',num2str(linkIndex),'.ply'],'tri');                
